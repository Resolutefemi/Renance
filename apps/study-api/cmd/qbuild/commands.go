...
func cmdBuild(o buildOpts) int {
	...
	packPath := filepath.Join(o.Outdir, "questions", filepath.FromSlash(packRelPath(spec.Code)))
	if err := os.MkdirAll(filepath.Dir(packPath), 0o755); err != nil {
		fmt.Fprintf(os.Stderr, "qbuild build: mkdir %s: %v\n", filepath.Dir(packPath), err)
		return 1
	}
	...
	srcDir := filepath.Join(o.Outdir, "src", o.Keysub)
	if err := os.MkdirAll(srcDir, 0o755); err != nil {
		fmt.Fprintf(os.Stderr, "qbuild build: mkdir %s: %v\n", srcDir, err)
		return 1
	}
	srcCopy := filepath.Join(srcDir, filepath.Base(spec.Source))
	...
	// Boot verification with the REAL loader: sha256 + counts + key harvest.
	if _, err := cbtdata.Load(o.Outdir); err != nil {
		fmt.Fprintf(os.Stderr, "qbuild build: boot-verify FAILED: %v\n", err)
		return 1
	}

	fmt.Printf("pack  → %s (answers embedded)\n", packPath)
