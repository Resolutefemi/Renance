...
func TestBuildRoundTripBootsRealLoader(t *testing.T) {
	...
	// The served paper must be sanitized, the harvested key complete.
	for _, q := range b.Questions {
		if q.Answer != "" || q.Explanation != "" {
			t.Fatalf("answer material leaked into served bundle: %+v", q)
		}
	}
	keys, ok := lib.KeysFor(spec.Code)
	if !ok || len(keys) != 2 || keys[spec.Code+"-0001"].Letter != "B" || keys[spec.Code+"-0002"].Letter != "A" {
		t.Fatalf("harvested key mismatch: %+v ok=%v", keys, ok)
	}
}
...
func TestPackRelPath(t *testing.T) {
	cases := map[string]string{
		...
		"uni-unilag-bio101-bank":                 "All_tertiary_Q/unilag/BIO101.json",
		"uni-mountain-top-gst111-bank":           "All_tertiary_Q/mountain-top/GST111.json",
		"uni-eksu-financial-sector-and-eco-bank": "All_tertiary_Q/eksu/financial-sector-and-eco.json",
		"uni-eksu-general-biology-i-bank":        "All_tertiary_Q/eksu/general-biology-i.json",
		"uni-oau-pq-mathematics-bank":            "POST_UTME/oau/mathematics.json",
		"uni-mountain-top-pq-english-bank":       "POST_UTME/mountain-top/english.json",
