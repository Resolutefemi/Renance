...
// GET /school/plan?schoolId= - the subscription summary: the school's
// current plan, when it expires, and the trial countdown the
// subscription page and the reminder mail both read.
func (s *Server) handleSchoolPlan(w http.ResponseWriter, r *http.Request) {
        _, sc, ok := s.memberAndSchool(w, r, r.URL.Query().Get("schoolId"))
        if !ok {
                return
        }
        now := time.Now().UTC()
        ...
        resp := map[string]any{
                "plan":      sc.Plan,
                "planSource": sc.PlanSource,
        }
        if sc.PlanExpiresAt != nil {
                resp["expiresAt"] = sc.PlanExpiresAt.UTC().Format(time.RFC3339)
                if days := int(time.Until(*sc.PlanExpiresAt).Hours() / 24); days >= 0 {
                        resp["daysLeft"] = days
                } else {
                        resp["daysLeft"] = 0
                }
        }
        if sc.TrialEndsAt != nil && sc.Plan == "trial" {
                resp["trialEndsAt"] = sc.TrialEndsAt.UTC().Format(time.RFC3339)
                if days := int(time.Until(*sc.TrialEndsAt).Hours() / 24); days >= 0 {
                        resp["trialDaysLeft"] = days
                } else {
                        resp["trialDaysLeft"] = 0
                }
        }
        _ = now
