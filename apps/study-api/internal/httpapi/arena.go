...
func arenaSink(st *store.Store) arena.MatchSink {
        return arena.SinkFunc(func(ctx context.Context, r arena.MatchResult) error {
                ...
                if err := st.SaveArenaMatch(ctx, rec); err != nil {
                        return err
                }
                // REN wallet (founder rule): a duel won pays 1 REN coin. Best
                // effort: a dropped coin never breaks the match record, and the
                // entitlement row lazily exists for anyone who fought a duel.
                if r.Status == "completed" && r.WinnerID != "" {
                        if _, err := st.EnsureEntitlement(ctx, r.WinnerID); err == nil {
                                if err := st.AddCoins(ctx, r.WinnerID, plans.ArenaWinCoins); err != nil {
                                        slog.Error("arena: ren coin award", "user", r.WinnerID, "err", err)
                                }
                        }
                }
                return nil
...
func (s *Server) handleArenaPlayers(w http.ResponseWriter, r *http.Request) {
        ...
        wins, err := s.store.ArenaWinsByUser(r.Context(), ids)
        if err != nil {
                s.log.Error("arena players wins", "err", err)
                fail(w, http.StatusInternalServerError, "internal", "could not load the active players")
                return
        }
        type playerRow struct {
                UserID   string `json:"userId"`
                Username string `json:"username"`
                Wins     int    `json:"renPoints"`
        }
        rows := make([]playerRow, 0, len(online))
        for _, p := range online {
                rows = append(rows, playerRow{UserID: p.UserID, Username: p.Username, Wins: wins[p.UserID]})
        }
