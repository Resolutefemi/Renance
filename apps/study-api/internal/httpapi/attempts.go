...
type submitRequest struct {
        ...
        // QuestionMs is the client's per-question dwell map (pacing
        // telemetry). It feeds the official UTME subject ledger's
        // time-used column; absent or oversized maps are simply ignored.
        QuestionMs map[string]int64 `json:"questionMs,omitempty"`
}
...
func (s *Server) handleSubmitAttempt(w http.ResponseWriter, r *http.Request) {
        ...
        for _, a := range req.Answers {
                ...
                if q.Type == "theory" {
                        // Essay questions carry no options: the essay text
                        // never leaves the client, the pick is a marker so
                        // the seat counts the question as addressed and the
                        // model answer unlocks in review.
                        if a.Selected != "" {
                                fail(w, http.StatusBadRequest, "invalid_choice",
                                        "theory question "+a.QuestionID+" takes no option")
                                return
                        }
                        picks = append(picks, store.Picked{QuestionID: a.QuestionID, Selected: ""})
                        continue
                }
        ...
        // Dwell map hygiene: only question ids that belong to this paper,
        // each capped at the paper's own wall-clock duration so a buggy
        // client can never print a subject that took longer than the exam.
        questionMs := map[string]int64{}
        if len(req.QuestionMs) > 0 && len(req.QuestionMs) <= 1000 {
                for id, ms := range req.QuestionMs {
                        if ms <= 0 {
                                continue
                        }
                        if _, onPaper := bundle.Question(id); !onPaper {
                                continue
                        }
                        cap := int64(24 * 60 * 60 * 1000)
                        if durationMs != nil {
                                cap = int64(*durationMs)
                        }
                        if ms > cap {
                                ms = cap
                        }
