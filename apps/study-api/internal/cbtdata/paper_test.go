...
func mustBodyBank(t *testing.T, lib *Library, body, slug string) *Bundle {
        t.Helper()
        b, ok := lib.Bundle(body + "-" + slug + "-bank")
        if !ok {
                t.Fatalf("bank missing: %s-%s-bank", body, slug)
        }
        return b
}

// The custom family is body-aware: waec-custom-/neco-custom- classify as
// custom papers, take y/n/t only, and compose from THAT body's banks.
func TestBodyCustomPapers(t *testing.T) {
        lib := loadRealLibrary(t)

        // classification
        for _, code := range []string{
                "waec-custom-biology-chemistry~n=40",
                "neco-custom-government~n=10",
                "waec-custom-english",
        } {
                if !IsCustomPaperCode(code) || !IsComposedPaperCode(code) {
                        t.Fatalf("%s must classify as a composed custom paper", code)
                }
        }
        if customBodyPrefix("waec-custom-biology") != BodyWaec || customBodyPrefix("neco-custom-english") != BodyNeco || customBodyPrefix("jamb-custom-english") != BodyJamb {
                t.Fatal("custom body prefix mapping broken")
        }
        // static packs must not classify
        for _, code := range []string{"waec-biology-bank", "neco-english-bank", "waec-custom-", "neco-custom-"} {
                if IsCustomPaperCode(code) {
                        t.Fatalf("%q must not classify as a custom paper", code)
                }
        }

        // grammar: parse + canonical round-trip
        code := "waec-custom-biology-chemistry~y=2019.n=40.t=60"
        spec, err := lib.ParsePaper(code)
        if err != nil {
                t.Fatalf("ParsePaper(%s): %v", code, err)
        }
        if spec.BodyName() != BodyWaec || spec.N != 40 || spec.Timer != 60 || spec.Years[0] != 2019 {
                t.Fatalf("parsed spec wrong: %+v", spec)
        }
        if spec.Encode() != code {
                t.Fatalf("non-canonical: %s != %s", spec.Encode(), code)
        }
        neco := "neco-custom-economics-government~n=20"
        spec, err = lib.ParsePaper(neco)
        if err != nil || spec.BodyName() != BodyNeco || spec.Encode() != neco {
                t.Fatalf("neco custom parse/encode broken: %+v %v", spec, err)
        }

        // JAMB-only English controls are refused on waec/neco
        for _, bad := range []string{
                "waec-custom-biology~comp=0",
                "waec-custom-english~compN=5",
                "waec-custom-english~enN=40",
                "neco-custom-english~nov=1",
        } {
                if _, err := lib.ParsePaper(bad); err == nil {
                        t.Errorf("%s: jamb-only param must be refused", bad)
                }
        }

        // composition from the real WAEC/NECO shelves
        paper, err := ComposePaper(spec, map[string]*Bundle{
                "economics":  mustBodyBank(t, lib, "neco", "economics"),
                "government": mustBodyBank(t, lib, "neco", "government"),
        })
        if err != nil {
                t.Fatalf("ComposePaper(neco): %v", err)
        }
        if paper.Body != "NECO" || paper.QuestionCount != 20 {
                t.Fatalf("neco paper body/count: %s/%d", paper.Body, paper.QuestionCount)
        }
        if len(paper.Sections) != 2 {
                t.Fatalf("neco paper sections: %d", len(paper.Sections))
        }
        if !strings.Contains(paper.Title, "NECO Practice") {
                t.Fatalf("neco paper title: %q", paper.Title)
        }

        waecCode := "waec-custom-biology-chemistry~n=10"
        wspec, err := lib.ParsePaper(waecCode)
        if err != nil {
                t.Fatalf("ParsePaper(%s): %v", waecCode, err)
        }
        wpaper, err := ComposePaper(wspec, map[string]*Bundle{
                "biology":   mustBodyBank(t, lib, "waec", "biology"),
                "chemistry": mustBodyBank(t, lib, "waec", "chemistry"),
        })
        if err != nil {
                t.Fatalf("ComposePaper(waec): %v", err)
        }
        if wpaper.Body != "WAEC" || wpaper.QuestionCount != 10 || !strings.Contains(wpaper.Title, "WASSCE Practice") {
                t.Fatalf("waec paper meta: %s %d %q", wpaper.Body, wpaper.QuestionCount, wpaper.Title)
        }
        // deterministic walk per code
        again, _ := ComposePaper(wspec, map[string]*Bundle{
                "biology":   mustBodyBank(t, lib, "waec", "biology"),
                "chemistry": mustBodyBank(t, lib, "waec", "chemistry"),
        })
        for i := range wpaper.Questions {
                if wpaper.Questions[i].ID != again.Questions[i].ID {
                        t.Fatalf("waec walk diverges at %d", i)
                }
        }

        // a waec-custom code still cannot pull jamb banks (body isolation)
        jspec, _ := lib.ParsePaper("waec-custom-biology~n=5")
        if _, err := ComposePaper(jspec, map[string]*Bundle{
                "biology": mustBodyBank(t, lib, "jamb", "biology"),
        }); err == nil {
                t.Fatal("waec paper composed from a jamb bank must fail")
        }
        ...
        epaper, err := ComposePaper(espec, map[string]*Bundle{
                "english": mustBodyBank(t, lib, "waec", "english"),
        })
        if err != nil {
                t.Fatalf("ComposePaper(waec english): %v", err)
        }
        if epaper.QuestionCount != 30 {
                t.Fatalf("waec english count: %d", epaper.QuestionCount)
        }
        againEn, _ := ComposePaper(espec, map[string]*Bundle{
                "english": mustBodyBank(t, lib, "waec", "english"),
        })
        for i := range epaper.Questions {
                if epaper.Questions[i].ID != againEn.Questions[i].ID {
                        t.Fatalf("waec english walk diverges at %d", i)
                }
        }
}
