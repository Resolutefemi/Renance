...
// SubjectRow is one subject's official ledger on a composite paper
// (jamb-mock-*): what the real UTME slip prints. Score is the subject's
// graded mark out of 100 - Use of English divides its correct count by
// 60, every other subject divides by 40 (2.5 marks per question).
// TimeMs is the dwell time the client banked for the subject's questions.
...
type Result struct {
        ...
        // Subjects carries the official per-subject ledger for composite
        // papers with sections; nil for every other pack.
        Subjects  []SubjectRow `json:"subjects,omitempty"`
...
func (s *Store) WriteResult(ctx context.Context, attemptID string, r *Result) error {
        ...
        var subjects []byte
        if len(r.Subjects) > 0 {
                subjects, err = json.Marshal(r.Subjects)
                if err != nil {
                        return fmt.Errorf("store: marshal subjects: %w", err)
                }
        }
        ...
        defer tx.Rollback(ctx)
        if _, err := tx.Exec(ctx, `
                INSERT INTO study.results (attempt_id, score, total, breakdown, subjects, graded_at)
                VALUES ($1, $2, $3, $4::jsonb, $5::jsonb, now())
                ON CONFLICT (attempt_id) DO UPDATE
                SET score = EXCLUDED.score, total = EXCLUDED.total,
                    breakdown = EXCLUDED.breakdown, subjects = EXCLUDED.subjects,
                    graded_at = now()`,
                attemptID, r.Score, r.Total, string(breakdown), nullableJSON(subjects)); err != nil {
...
func (s *Store) ResultByAttempt(ctx context.Context, attemptID string) (*Result, error) {
        var breakdown, subjects []byte
        r := &Result{}
        err := s.Pool.QueryRow(ctx, `
                SELECT score, total, breakdown, subjects, graded_at
                FROM study.results WHERE attempt_id = $1`, attemptID,
        ).Scan(&r.Score, &r.Total, &breakdown, &subjects, &r.GradedAt)
        if errors.Is(err, pgx.ErrNoRows) {
                return nil, nil
        }
        if err != nil {
...
// nullableJSON maps an empty JSON blob to SQL NULL so optional jsonb
// columns (result subjects) stay NULL instead of the string "null".
