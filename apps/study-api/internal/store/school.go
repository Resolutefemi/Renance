...
type School struct {
        ...
        // Subscription: plan (trial|free|basic|standard|premium), the
        // plan's expiry, the trial's end and what granted it. The owner can
        // flip these straight from the Neon console.
...
type SchoolSubject struct {
        ...
        Level      string `json:"level"`
        Seq        int    `json:"seq"`
        Department string `json:"department"` // '' | art | science | commercial (senior band)
        IsCore     bool   `json:"isCore"`     // compulsory for every SSS student
}
...
func (s *Store) CreateSchool(ctx context.Context, name, schoolType, ownerUserID, ownerFullName string) (*School, *SchoolMember, error) {
        ...
        // Every new school starts on the 30-day trial: everything except
        // PDF downloads, no card, and no surprise when it ends (the
        // subscription page and the reminder mail count the days down).
        err = tx.QueryRow(ctx, `
                INSERT INTO school.schools (name, school_type, owner_user_id, plan, trial_ends_at)
...
func (s *Store) SchoolByID(ctx context.Context, id string) (*School, error) {
        sc := &School{}
        err := s.Pool.QueryRow(ctx, `
                SELECT id, name, school_type, address, COALESCE(logo_url, ''),
                       COALESCE(plan, 'trial'), plan_expires_at, trial_ends_at, COALESCE(plan_source, ''), created_at
                FROM school.schools WHERE id = $1`, id,
        ).Scan(&sc.ID, &sc.Name, &sc.SchoolType, &sc.Address, &sc.LogoURL,
                &sc.Plan, &sc.PlanExpiresAt, &sc.TrialEndsAt, &sc.PlanSource, &sc.CreatedAt)
        if errors.Is(err, pgx.ErrNoRows) {
                return nil, nil
        }
        if err != nil {
...
func (s *Store) SeedCurriculum(ctx context.Context, schoolID string) (classes, subjects int, err error) {
        ...
        for _, sub := range school.Subjects {
                var id string
                err := tx.QueryRow(ctx, `
                        INSERT INTO school.subjects (school_id, name, code, level, seq, department, is_core)
                        VALUES ($1, $2, $3, $4, $5, $6, $7)
                        ON CONFLICT (school_id, name, level) DO UPDATE SET
                          seq = EXCLUDED.seq, department = EXCLUDED.department,
                          is_core = EXCLUDED.is_core
                        RETURNING id`, schoolID, sub.Name, sub.Code, sub.Level, sub.Seq, sub.Department, sub.IsCore,
                ).Scan(&id)
                if err != nil {
...
func (s *Store) ListSubjects(ctx context.Context, schoolID string) ([]SchoolSubject, error) {
        rows, err := s.Pool.Query(ctx, `
                SELECT id, school_id, name, code, level, seq,
                       COALESCE(department, ''), COALESCE(is_core, false)
                FROM school.subjects WHERE school_id = $1 ORDER BY seq`, schoolID)
        if err != nil {
                return nil, err
        }
        defer rows.Close()
...
// scanSubject reads the shared subject column order (ListSubjects and
// CreateSubject both use it).
func scanSubject(row pgx.Row) (*SchoolSubject, error) {
        var sub SchoolSubject
        err := row.Scan(&sub.ID, &sub.SchoolID, &sub.Name, &sub.Code, &sub.Level, &sub.Seq,
                &sub.Department, &sub.IsCore)
        if err != nil {
                return nil, err
        }
        return &sub, nil
...
func (s *Store) ListStudents(ctx context.Context, schoolID, classID string) ([]SchoolStudent, error) {
        base := `SELECT id, school_id, COALESCE(class_id::text, ''), full_name, admission_no, sex, session,
                       COALESCE(guardian_name, ''), COALESCE(dob, '')
        ...
        for rows.Next() {
                ...
                if err := rows.Scan(&st.ID, &st.SchoolID, &st.ClassID, &st.FullName, &st.AdmissionNo, &st.Sex, &st.Session,
                        &st.GuardianName, &st.DOB); err != nil {
                        return nil, err
                }
                out = append(out, st)
        }
...
func (s *Store) ResultsGrid(ctx context.Context, schoolID, classID string, term int, session string) ([]SchoolResult, error) {
        ...
        // Per-student subject offerings (SSS tracks): an explicit list
        // narrows the report card to exactly those subjects.
        offering := map[string]map[string]bool{}
        orows, err := s.Pool.Query(ctx, `
                SELECT ss.student_id, ss.subject_id
                FROM school.student_subjects ss
                JOIN school.students st ON st.id = ss.student_id
                WHERE st.school_id = $1 AND st.class_id = $2`, schoolID, classID)
        if err == nil {
                for orows.Next() {
                        var sid, subID string
                        if err := orows.Scan(&sid, &subID); err != nil {
                                orows.Close()
                                return nil, err
                        }
                        if offering[sid] == nil {
                                offering[sid] = map[string]bool{}
                        }
                        offering[sid][subID] = true
                }
                orows.Close()
        ...
        for _, st := range students {
                ...
                default:
                        ...
                        res.Items = filterByOffering(items, offering[st.ID])
                }
                out = append(out, res)
        }
        return out, nil
...
// filterByOffering keeps only the subjects a student explicitly offers.
// An empty offering map means "follow the class" and keeps everything.
func filterByOffering(items []SchoolResultItem, allow map[string]bool) []SchoolResultItem {
        if len(allow) == 0 {
                return items
        }
        out := items[:0:0]
        for _, it := range items {
                if allow[it.SubjectID] {
                        out = append(out, it)
                }
        }
        return out
...
func (s *Store) ResultSheet(ctx context.Context, schoolID, studentID string, term int, session string) (*SchoolResult, error) {
        ...
        // School identity: stamped on the report card header.
        var schoolName, schoolLogo string
        _ = s.Pool.QueryRow(ctx, `
                SELECT name, COALESCE(logo_url, '') FROM school.schools WHERE id = $1`, schoolID,
        ).Scan(&schoolName, &schoolLogo)
        ...
        offeringSet, err := s.StudentSubjectIDs(ctx, studentID)
        if err != nil {
                return nil, err
        }
        allow := map[string]bool{}
        for _, sid := range offeringSet {
                allow[sid] = true
        }
        ...
        res := &SchoolResult{
                ...
                ClassID: classID, ClassName: className, Term: term, Session: session,
                SchoolName: schoolName, SchoolLogoURL: schoolLogo,
        }
        ...
        if err == nil {
                res.Items = filterByOffering(res.Items, allow)
        }
        return res, err
}
...
func (s *Store) CheckResultByPIN(ctx context.Context, pin string, term int, session string) (*SchoolResult, error) {
        ...
        // results carries no school_id of its own - reach it through the student.
        err := s.Pool.QueryRow(ctx, `
                SELECT r.id, r.student_id, st.school_id
                FROM school.results r
                JOIN school.students st ON st.id = r.student_id
                WHERE r.pin = $1 AND r.term = $2 AND r.session = $3 AND r.status = 'finalized'`,
                pin, term, session,
